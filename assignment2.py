...
def HumanPlayer(Tag, Board):
    ...
    while True:
        try:
            row_choice = int(input("Choose a row reference: "))
            col_choice = int(input("Choose a column reference: "))
        except ValueError:
            print("That's not a valid input, please enter an integer value")
            continue
        else:
            break
        
    while IsSpaceFree(Board, row_choice, col_choice) == False:
        print("Space is not free, choose again")
        row_choice = int(input("Choose a row reference: "))
        col_choice = int(input("Choose a column reference: "))

    choice = (row_choice,col_choice)
    return choice

    # print("You chose to place your mark at: Board[{}][{}]".format(row_choice, col_choice))
    # Board[row_choice][col_choice] = Tag
...
def ComputerPlayer(Tag, Board):
    ...
    while IsSpaceFree(Board, row, col) == False:
        row = random.randint(0,2)
        col = random.randint(0,2)

    choice = (row, col)
    print("THIS ARE THE VALUESSSSSSSSS", choice)
    return choice
...
def TicTacToeGame():
    ...
    # Choice is shown within the Players or Computers function
    done = False
    while not done:
        # First player goes
        ...
        # Redraw Board
        DrawBoard(Board)
        # Judge whether there is a winner or game ended in tie
        #Judge(Board)
        if Judge(Board) > 0:
            print("Game ended")
            # done = True
